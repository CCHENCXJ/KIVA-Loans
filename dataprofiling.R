...
dataPath <-"/Users/jerrychang/Desktop/UChicago/MScA31012/final_project"

#load data
loans_original <- read_csv(paste(dataPath,'kiva_loans.csv',sep = '/'))
country_stats<-read_csv(paste(dataPath,'country_stats.csv',sep = '/'))
mpi_on_regions<- read_excel(paste(dataPath,'mpi_on_regions.xlsx',sep = '/')) #for region_mpi
multipoverty<-read_csv(paste(dataPath,'MPI_subnational.csv',sep = '/')) #for country_mpi
loan_theme_by_region<-read_csv(paste(dataPath,'loan_themes_by_region.csv',sep = '/'))
loan_theme_id<-read_csv(paste(dataPath,'loan_theme_ids.csv',sep = '/'))
UN<-read_csv(paste(dataPath,'kiva_country_profile_variables.csv',sep = '/'))

#descriptive statistics
summary(loans_original)
sd(loans_original$funded_amount)
summary(country_stats)
summary(mpi_on_regions)
summary(loan_theme_by_region)
summary(multipoverty)
summary(loan_theme_id)
summary(UN)

#null value identification
x<- colMeans(is.na(loans_original))
distribution<- x[x>0]
t<-data.frame(distribution)
colnames(t) <- "Percentage"
t$Percentage <- t$Percentage *100
d = data.frame(t, stringsAsFactors = TRUE)
dt <- datatable(d, filter = 'bottom', options = list(pageLength = 6))
dt

x<- colMeans(is.na(country_stats))
distribution<- x[x>0]
t<-data.frame(distribution)
colnames(t) <- "Percentage"
t$Percentage <- t$Percentage *100
...
x<- colMeans(is.na(mpi_on_regions))
distribution<- x[x>0]
t<-data.frame(distribution)
colnames(t) <- "Percentage"
t$Percentage <- t$Percentage *100
...
x<- colMeans(is.na(loan_theme_by_region))
distribution<- x[x>0]
t<-data.frame(distribution)
colnames(t) <- "Percentage"
t$Percentage <- t$Percentage *100
...
x<- colMeans(is.na(loan_theme_id))
distribution<- x[x>0]
t<-data.frame(distribution)
colnames(t) <- "Percentage"
t$Percentage <- t$Percentage *100
...
##sex ratio
loans_original$borr <- if_else((loans_original$borrower_genders == "female"), "female","male")
k<-data.frame(table(loans_original$country[loans_original$borr == "female"]))
k<-as.data.frame(table(loans_original$borr))
...
## gender based loan distribution for each sector
x<- data.frame(table(loans_original$sector[loans_original$borr == "female"]))
k<- data.frame(table(loans_original$sector))
